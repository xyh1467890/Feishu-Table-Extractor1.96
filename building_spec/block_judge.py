...
class BlockSingleJudge:
    ...
    def send_request(self, queries, before_base_token, after_base_tokens=None, 
        ...
        # 构建请求体 - Block 特殊格式（支持多轮对话，不需要 mode）
        data = {
            "agent": agent,
            "cases": [{
                "id": "block_multi_turn_001",
                "query": queries,
...
class BlockBatchJudge:
    ...
    def read_cases_from_file(self, file_path):
        """
        从文件读取 Block 测试用例（支持 CSV 和 Excel 格式）
    ...
    def send_request(self, cases, agent="block", concurrency=5):
        """
        发送 Block 批量测试请求
